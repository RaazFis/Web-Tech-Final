    ...
    <table>
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<button class='share-button'>Status</button>";
                echo "<p class='comment-status'></p>";
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        commentButtons.forEach(function(button) {
            button.addEventListener('click', function() {
                ...
                var commentStatus = button.nextElementSibling;
                var commentsContainer = button.nextElementSibling.nextElementSibling;

                var comment = commentBox.value.trim();

                if (comment === '') {
                    commentStatus.textContent = "Nothing is commented";
                } else {
                    commentStatus.textContent = "Commented successfully";
